...
dirname = './Experiment-SIGIR-LEARNING'
print dirname
os.mkdir(dirname)
							...
							for elStrategy in listStrategy:
								if "group" in elType:
									...
									with open(dirname+'/'+sbatch_filename, 'w') as the_file:
										...
										if elFeature == '' : the_file.write('srun /logiciels/Python-2.7/bin/python2.7 /projets/sig/PROJET/PRINCESS/code/princess/princess.py -t '+elType+' -r '+elRound+' -b 0.2 -c '+elCollection+' -l '+elLife+' -i '+elImpact+' -d -g 5 -n 0 -s '+elStrategy+"\n")
										else : the_file.write('srun /logiciels/Python-2.7/bin/python2.7 /projets/sig/PROJET/PRINCESS/code/princess/princess.py -t '+elType+' -r '+elRound+' -b 0.2 -c '+elCollection+' -l '+elLife+' -i '+elImpact+' -g 5 -n 0 -s '+elStrategy+' -d -a -f '+elFeature+"\n")
