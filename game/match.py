...
class Match(object):
	...
	def __init__(self,a,b,impact=0,health=0,nbFeat=0,strategy=1,start=0,optim="order"):
		...
		self.strategy_feat = ['f48', 'f17', 'f19', 'f16', 'f46', 'f9', 'f21', 'f3', 'f39', 'f7', 'f40', 'f6', 'f37', 'f42', 'f2', 'f15', 'f25', 'f33', 'f36', 'f10', 'f30', 'f51', 'f28', 'f43', 'f45', 'f34', 'f24', 'f13', 'f50', 'f27', 'f31', 'f1', 'f35', 'f14', 'f47', 'f41', 'f4', 'f22', 'f12', 'f8', 'f26', 'f44']
	...
	def elaborated_match(self):
		...
		while feats and health_a > 0 and health_b > 0 :
			if current_player == self.doc_a :
				...
				while not any(d.name == strategy_a[0].name for d in self.doc_b.features) or strategy_a[0].name == 'f0' or not strategy_a[0].name in feats: 
				...
				# n'existe pas dans b ou egal a 0 ou deja joue
				while not any(d.name == strategy_b[0].name for d in self.doc_a.features) or strategy_b[0].name == 'f0' or not strategy_b[0].name in feats: 
