...
functionMapping = {"f1" : features.tf,"f2" : features.tf,"f3" : features.tf, "f4" : features.idf,"f5":features.idf,"f6" : features.idf, "f13":features.normalizedbysize,"f14":features.normalizedbysize,"f15":features.normalizedbysize, "f16":features.normalizedbysize, "f17":features.normalizedbysize,"f18":features.normalizedbysize, "f19":features.normalizedbysize, "f20":features.normalizedbysize,"f21":features.normalizedbysize, "f28":features.runquerymodel, "f29":features.runquerymodel, "f30":features.runquerymodel, "f31":features.runquerymodel, "f32":features.runquerymodel,"f33":features.runquerymodel,"f34":features.runquerymodel, "f35":features.runquerymodel,"f36":features.runquerymodel,"f37":features.runquerymodel,"f38":features.runquerymodel, "f39":features.runquerymodel,"f40":features.runquerymodel, "f41":features.runquerymodel,"f42":features.runquerymodel,"f43":features.runquerymodel,"f44":features.runquerymodel,"f45":features.runquerymodel}
...
for q in list(queryTerms.keys()) :  # Initialisation
    featuresValues[q] = {}


for currIndex in list(indexValues.keys()) :  # Pour chaque type d'index
    ...
    for q in list(queryTerms.keys()) :  # Pour chaque requete
            ...
            for i in range(0,len(infoFeatures)):
                if infoFeatures[i] != {} :
                    ...
                    if (infoFeatures[i]["index"] ==  currIndex and infoFeatures[i]["dependant"] == "term"):
                        termInDoc[t] =[]
                        if i not in list(range(49,52)) and feat not in list(termDependant.keys()):
            ...
            with open(pathTempFilteredFile) as tempFile:
                ...
                for currLine in tempDocs :
                    ...
                    if (currLine != ""):
                        ...
                        if currDoc not in list(featuresValues[q].keys()):
                        ...
                        for i in range(0,len(infoFeatures)) :
                            if infoFeatures[i] != {} :
                                ...
                                if (infoFeatures[i]["index"] ==  currIndex and infoFeatures[i]["dependant"] == "document-term"):
                                    if feat in list(functionMapping.keys()):
                                        #os.system('read -s -n 1 -p "Press any key to continue..."')
                                        if feat not in list(featuresValues[q][currDoc].keys()):
...
for q in list(featuresValues.keys()):   # q => la requete courante / indexe une collection de documents
    maxFeat[q] = {}
    minFeat[q] = {}
    valFeat[q] = {}
    #print "nbDocs => "+str(len(featuresValues[q].keys() ))
    for doc in list(featuresValues[q].keys()) : # doc un document / indexe un ensemble de features

        for f in list(featuresValues[q][doc].keys()) : # f la feature
...
if normType == "discWidth":
    for q in list(valFeat.keys()) :
        dictBounds[q]={}
        for f in list(valFeat[q].keys()) : # f la feature
